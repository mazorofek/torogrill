  ...
  return (
    ...
    >
      <button
        ...
        aria-label="סגירת אפשרויות משלוח"
        onClick={onClose}
      />
      <div className="relative w-full max-w-sm border border-primary/45 bg-[#070707] p-6 shadow-2xl">
        <button
          type="button"
          onClick={onClose}
          className="absolute left-4 top-4 flex h-9 w-9 items-center justify-center border border-white/15 text-white/65 transition-colors hover:border-primary hover:text-white"
          aria-label="סגירה"
        ...
        <p className="mb-6 text-sm text-white/50">בחרו מאיפה להזמין</p>
        ...
        <div className="space-y-3">
          {deliveryOptions.map((option) => (
            <a
              ...
              data-testid={`delivery-option-${option.name}`}
            >
